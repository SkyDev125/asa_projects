...
unsigned** build_table(unsigned n) {
    ...
    for (unsigned i = 0; i < n; i++) tab[i] = new unsigned[n];
    ...
    for (unsigned i = 0; i < n; i++)
        for (unsigned j = 0; j < n; j++) tab[i][j] = uniform(rng);
    return tab;
}
...
unsigned* build_sequence(unsigned** tab, unsigned n, unsigned m) {
    ...
    mt19937 rng(dev());
    std::uniform_int_distribution<std::mt19937::result_type> uniform(1, n);
    for (unsigned i = 0; i < m; i++) seq[i] = uniform(rng);

    // compute a valid output by eliminating randomly chosen neighbours
    vector<unsigned> tmp(seq, seq + m);
    while (tmp.size() > 1) {
        std::uniform_int_distribution<std::mt19937::result_type> uniform(
            0, tmp.size() - 2);
...
void print_output(unsigned** tab, unsigned n, unsigned* seq, unsigned m) {
    ...
    for (unsigned i = 0; i < m; i++) cout << (i == 0 ? "" : " ") << seq[i];
    cout << endl;
    cout << seq[m] << endl;
}

void cleanup(unsigned** tab, unsigned n, unsigned* seq, unsigned m) {
    for (unsigned i = 0; i < n; i++) delete[] tab[i];
    delete[] tab;
    delete[] seq;
}
...
int main(int argc, char* argv[]) {
    if (argc == 1) {
        ...
        cout << "The examples generated always have a solution." << endl
             << endl;
        cout << "Usage: " << argv[0] << " <table size> <sequence size>" << endl
             << endl;
        return 0;
    }
